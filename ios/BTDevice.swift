...
protocol BTDataReceivedDelegate {
...
/**
 ...
 BTDevice is responsible for maintaining the connection Stream(s) with the device, using
 ...
 to the delegate instead of storing it within the BTDevice buffer.
 */
class BTDevice: NSObject, StreamDelegate {
    ...
    var receivedDelegate:BTDataReceivedDelegate?
    
    /**
     Initialize the BTDevice with an EAAccessory.
     
     - parameter _: the accessory backing the BTDevice
     */
    ...
    func connect(
        ...
        NSLog("(BTDevice:connect) Attempting Bluetooth connection to %@", accessory.serialNumber)
    ...
    func connect(
        ...
        connect(protocolString: protocolString,
                bytesPerSend: BTDevice.MAX_BYTES_PER_SEND,
                bytesPerReceive: BTDevice.MAX_BYTES_PER_RECEIVE)
    }
     ...
     */
    func disconnect() {
        NSLog("(BTDevice:disconnect) Attempting disconnect from devices %@", accessory.serialNumber)
        if let currentSession = session {
    ...
    func bytesAvailable() -> Int {
        NSLog("(BTDevice:hasBytesAvailable) Checking for available bytes on devices %@", accessory.serialNumber)
    ...
    func writeToDevice(_ message:String) {
        NSLog("(BTDevice:writeToDevice) Writing %@ to device %@", message, accessory.serialNumber)
    ...
    func readFromDevice() -> String? {
        NSLog("(BTDevice:readFromDevice) Reading entire content from device %@", accessory.serialNumber)
    ...
    func readFromDevice(withDelimiter delimiter:String) -> String? {
        NSLog("(BTDevice:readFromDevice) Reading device %@ until delimiter %@", accessory.serialNumber, delimiter)
        let content = String(data: inBuffer, encoding: .utf8)!
        var message:String?
    ...
    /**
     ...
     BTDevice.onDataRead listener, then it will be notified instead of appending
    ...
    private func readData() {
        ...
        if (numBytesRead <= 0) {
            NSLog("(BTDevice:readData) No buffer")
            return
        }
    ...
    private func writeData() {
        if (outBuffer.isEmpty) {
            NSLog("(BTDevice:writeData) No buffer data scheduled for deliver")
            return
        }
        
        let len:Int = (outBuffer.count > maxBytesPerSend) ? maxBytesPerSend : outBuffer.count
        NSLog("(BTDevice:writeData) Attempting to send %d bytes to the device", len)
        ...
        NSLog("(BTDevice:writeData) Sent %d bytes to the device", bytesWritten)
    }
