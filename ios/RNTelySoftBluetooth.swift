...
/**
 Implementation of the RNTelySoftBluetooth React Native module.  For information on how this
 ...
 RNTelySoftBluetooth is responsible for interacting with the ExternalAccessory framework
 and providing wrappers for listing, connecting, reading, writing, etc.  The actual
 communication has been handed off to the BTDevice class - allowing (in the future)
 more that one BTDevice to be connected at one time.
 ...
 BTDevice side.  This means that the responsiblity of converting and managing
 data is done in Javascript/client rather than in the module.
 */
@objc(RNTelySoftBluetooth)
class RNTelySoftBluetooth : RCTEventEmitter {
    ...
    var peripheral:BTDevice?
    var delimiter:String
    var encoding:String.Encoding
    var readObserving:Bool
    
    /**
     Initialize the RNTelySoftBluetooth using the default delimiter and charset.
     */
    override init() {
    ...
    /**
     Initialize the RNTelySoftBluetooth module with a custom default delimiter and character set.
     */
    ...
    func accessoryDidConnect(_ notification:Notification) {
        ...
        if let connected: EAAccessory = notification.userInfo!["EAAccessoryKey"] as? EAAccessory {
            sendEvent(withName: BTEvent.BLUETOOTH_CONNECTED.rawValue, body: BTDevice(connected).asDictionary())
        }
    }
    ...
    /**
     ...
     a connection to it.  This is done by providing the self to the BTDevice
    ...
    func connect(
        ...
        // using the EAAccessoryManager, if found we create a new BTDevice
        // which will be responsible for managing our connection
        for accessory in eaManager.connectedAccessories {
            if accessory.serialNumber == deviceId {
                peripheral = BTDevice(accessory)
                peripheral!.receivedDelegate = self
                break
            }
        }
        
        if let toConnect:BTDevice = peripheral {
            ...
            if let protocolString:String = determineProtocolString(forDevice: toConnect) {
                NSLog("(RNTelySoftBluetooth:connect) Connecting to %@ with %@", toConnect.accessory.name, protocolString)
    ...
    private func determineProtocolString(forDevice device:BTDevice) -> String? {
    ...
    func disconnect(
        ...
        if let toDisconnect = peripheral {
            NSLog("(RNTelySoftBluetooth:disconnect) Disconnecting %@", toDisconnect.accessory.name)
            toDisconnect.disconnect()
            peripheral = nil
        }
        resolve(true)
    ...
    func isConnected(_ resolve: RCTPromiseResolveBlock,
        ...
        NSLog("(RNTelySoftBluetooth:isConnected) isConnected %@", peripheral?.accessory.name ?? "nil")
    ...
    func getConnectedDevice(
        _ resolve: RCTPromiseResolveBlock,
        rejecter reject: RCTPromiseRejectBlock) -> Void {
        NSLog("(RNTelySoftBluetooth:getConnectedDevice) Determine whether %@ is connected", peripheral?.accessory.name ?? "nil")
        if peripheral != nil {
            resolve(peripheral?.asDictionary())
        } else {
    ...
    func writeToDevice(
        ...
        NSLog("(RNTelySoftBluetooth:writeToDevice) Writing %@ to device %@", message, peripheral?.accessory.name ?? "nil")
    ...
    /**
     Allows React Native to inform RNTelySoftBluetooth whether there are active READ listeners.  The current implementation
...
extension RNTelySoftBluetooth : BTDataReceivedDelegate {
    
    /**
     Determine whether React Native is observing read events, if so created the appropriate object and send the event
     to React Native.
     */
    func onReceivedData(fromDevice: BTDevice, receivedData: Data) -> Data {
        ...
        if let data = String(data: receivedData, encoding: encoding) {
            ...
            for index in indexes {
                ...
                NSLog("(RNTelySoftBluetooth:onReceiveData) Sending READ with data: %@", message)
                let btMessage:BTMessage = BTMessage<String>(fromDevice: fromDevice, data: message)
                sendEvent(withName: BTEvent.READ.rawValue, body: btMessage.asDictionary())
