 ...
 *
 */
public class RNTelySoftBluetoothModule
        extends ReactContextBaseJavaModule
        implements ActivityEventListener,
        LifecycleEventListener,
        RCTEventEmitter,
        BTEventListener,
        BTDiscoveryReceiver.DiscoveryCompleteListener {

  private static final String TAG = RNTelySoftBluetoothModule.class.getSimpleName();
   ...
   */
  private RNTelySoftBluetoothService mBluetoothService;
   ...
   */
  private final BroadcastReceiver mBTStateReceiver = new BTStateReceiver(this);
   ...
   */
  private final BroadcastReceiver mBTConnectionReceiver = new BTConnectionReceiver(this);
   ...
   */
  private BroadcastReceiver mBTDiscoveryReceiver;
   ...
   * into the RNTelySoftBluetoothService to create a more similar pattern to IOS.
   */
  ...
  private Promise mDiscoverablePromise;
  ...
  /**
   * Creates a new RNTelySoftBluetoothModule using the standard delimiter and charset.
   *
   * @param reactContext react native context
   */
  public RNTelySoftBluetoothModule(ReactApplicationContext reactContext) {
    this(reactContext, "\n", CommonCharsets.LATIN.charset());
  }

  /**
   * Creates a new {@link RNTelySoftBluetoothModule} with a custom default delimiter and charset.
   * Attempts to get the BluetoothAdapter from the Android system and initialize the
   * {@link RNTelySoftBluetoothService}.  Finally sends appropriate events to Javascript and registers
  ...
  public RNTelySoftBluetoothModule(ReactApplicationContext reactContext,
    ...
    if (mBluetoothService == null) {
      mBluetoothService = new RNTelySoftBluetoothService(this);
    }
  ...
  public void onActivityResult(Activity activity, int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == BluetoothRequest.DISCOVERABLE.code) {
      if (resultCode == Activity.RESULT_OK) {
        if (D) Log.d(TAG, "User Discoverable Bluetooth");
        if (mDiscoverablePromise != null) {
          mDiscoverablePromise.resolve(true);
        }
      } else {
        if (D) Log.d(TAG, "User did *NOT* Discoverable Bluetooth");
        if (mDiscoverablePromise != null) {
          mDiscoverablePromise.reject(new Exception("User did not Discoverable Bluetooth"));
        }
      }
      mDiscoverablePromise = null;
    }
  ...
  public void requestDiscoverable(Promise promise) {
    ...
    if (mBluetoothAdapter != null && mBluetoothAdapter.isEnabled()) {
      ...
      Activity activity = getCurrentActivity();
      mDiscoverablePromise = promise;
      Intent intent = new Intent(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);
      intent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 300);
      if (activity != null) {
        activity.startActivityForResult(intent, BluetoothRequest.DISCOVERABLE.code);
      } else {
        Exception e = new Exception("Cannot start activity");
        Log.e(TAG, "Cannot start activity", e);
        mDiscoverablePromise.reject(e);
        mDiscoverablePromise = null;
        onError(e);
      }
    }
  ...
  public void getDeviceMacAddress(Promise promise) {
    String bluetoothAddress = android.provider.Settings.Secure.getString(this.mReactContext.getContentResolver(), "bluetooth_address");
    if (bluetoothAddress == null) {
      bluetoothAddress = mBluetoothAdapter.getAddress();
    }
    promise.resolve(bluetoothAddress != null ? bluetoothAddress : "");
  }
  ...
  public void list(Promise promise) {
    ...
    if (mBluetoothAdapter != null) {
      ...
      for (BluetoothDevice rawDevice : bondedDevices) {
        deviceList.pushMap(new BTDevice(rawDevice).map());
      }
    }
  ...
  /**
   * Attempt to discover unpaired devices.  Resolves when the BTDiscoveryReceiver
   * returns.
   *
  ...
  public void discoverDevices(final Promise promise) {
    ...
    if (mBluetoothAdapter != null) {
      ...
      mBTDiscoveryReceiver = new BTDiscoveryReceiver(this);

      mReactContext.registerReceiver(mBTDiscoveryReceiver, intentFilter);
  ...
  @Override
  public void onDiscoveryComplete(Collection<BTDevice> unpairedDevices) {
    WritableArray deviceArray = Arguments.createArray();
    for (BTDevice device : unpairedDevices) {
    ...
    try {
      mReactContext.unregisterReceiver(mBTDiscoveryReceiver);
  ...
  public void getConnectedDevice(Promise promise) {
    if (mBluetoothService.isConnected()) {
      promise.resolve(new BTDevice(mBluetoothService.connectedDevice()).map());
    }
  ...
  public void onConnectionSuccess(BluetoothDevice device) {
    ...
    // device.
    BTDevice nativeDevice = new BTDevice(device);

    sendEvent(BTEvent.CONNECTION_SUCCESS.code, nativeDevice.map());
  ...
  public void onConnectionFailed(BluetoothDevice device, Throwable reason) {
    ...
    if (mConnectedPromise != null) {
      mConnectedPromise.reject(new Exception("Connection unsuccessful"));
      mConnectedPromise = null;
    }
  }
  ...
  public void onConnectionLost (BluetoothDevice device, Throwable reason) {
    ...
    params.putMap("device", new BTDevice(device).map());
  ...
  public void onError (Throwable e) {
    ...
    params.putMap("device", new BTDevice(mBluetoothService.connectedDevice()).map());
  ...
  public void onReceivedData(BluetoothDevice device, byte[] data) {
    ...
    while ((message = readUntil(this.mDelimiter)) != null) {
      BTMessage btMessage
              = new BTMessage<>(new BTDevice(mBluetoothService.connectedDevice()).map(), message);
      sendEvent(BTEvent.READ.code, btMessage.asMap());
    }
  }
  ...
  private void registerBluetoothReceivers() {
    ...
    mReactContext.registerReceiver(mBTStateReceiver, stateIntent);

    IntentFilter connIntent = new IntentFilter();
    connIntent.addAction(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED);
    mReactContext.registerReceiver(mBTConnectionReceiver, connIntent);
  }
  ...
  private void unregisterBluetoothReceivers() {
    mReactContext.unregisterReceiver(mBTStateReceiver);
    mReactContext.unregisterReceiver(mBTConnectionReceiver);
